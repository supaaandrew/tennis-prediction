...
`MatchHistoryIndex` (strict `< as_of`); each match's serve row is read via
`MatchStatRepository.get(match_id, player_id)`.
"""
...
class ServeReturnExtractor:
    ...
    def _aggregates(
        ...
        the player's prior matches; each match's serve row is fetched once and
        added to career, and to recent when its instant is within the window."""
        career = _ServeAggregate()
        recent = _ServeAggregate()
        for m in self._history.player_matches_before(player_id=player_id, as_of=as_of):
            stat = self._stat_repo.get(match_id=m.match_id, player_id=player_id)
            if stat is None:
                continue
            career.add(stat)
