...
⬜ M1b               Modeling Agent (scoring) — stacking meta + calibration (→partial) + edge (Shin/proportional) + Kelly (per-match + H12 same-day) + predictions write; real H1 noise injection; categorical-code pin. Lock §M22 (noise sigma)/§M23 (cat pin)/§M24 (Kelly same-day) pre-build
