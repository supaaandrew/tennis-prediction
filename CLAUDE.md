...
⬜ Monitor Agent     agents/monitor/ (AGENTS.md A13) — runs post-briefing regardless of upstream status; ECE/PSI/ROI over config.monitor.windows_days into pipeline_runs.metrics. Owns the deferred metrics (§M18 bulk_read_failures, §M21 feature_hash dtype-drift/PSI). See spec.md
⬜ Orchestrator wiring  DI adapter-factory wiring + cron shim → DailyPipeline.run_once() (thin, deferred). Owns §N5 email-delivery idempotency/outbox.
