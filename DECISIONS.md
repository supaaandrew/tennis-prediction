...
│   ├── models/                # M1a — Modeling Agent ML internals (one module per concern)
│   │   ├── __init__.py
│   │   ├── feature_set.py     # M1a — resolve model X (market family + forecast bucket + backtest-only EXCLUDED) + feature_hash (§M20a/b/d, §M21a/b)
│   │   ├── assembly.py        # M1a — feature_matrix+labels → (X,y,meta); drop walkover/invalid-winner, keep retirement; native cat/missing (§M20a, §M21f)
│   │   ├── splits.py          # M1a — walk-forward CV; tail carved BEFORE folds; both-sides tournament embargo incl. train↔tail seam (§M20c, §M21c/e)
│   │   ├── base_learners.py   # M1a — XGB/LGBM native-cat + train-subsplit early stop; OOF; single-class skip + per-fold heartbeat (§M20a, §M21d)
│   │   ├── noise.py           # M1a — H1 apply_noise() no-op stub (real per-bucket injection deferred to M1b)
│   │   └── artifacts.py       # M1a — joblib persistence + µs-resolution version mint (§M20d)
│   └── agents/
│       ├── __init__.py
│       ├── data/              # P7 — DataAgent (daily ingest orchestrator)
│       │   ├── __init__.py
│       │   └── agent.py       # DataAgent(Agent): §J2 order, fault isolation, §L11 venue geocode pass, §L
│       ├── modeling/          # M1a — Modeling Agent (train → model_registry)
│       │   ├── __init__.py
│       │   └── agent.py       # M1a — ModelingAgent(Agent): assemble→split→CV→train→register+activate; heartbeat-in-loop, OOF gate (§M16-shape, §M20/§M21)
...
Next session resumes at **Modeling Agent M1b** (`agents/modeling/`): the stacking meta-learner + calibration (Platt/isotonic on the held-out tail carved by M1a's `splits`; tail below `min_calibration_samples` → `partial`) + edge vs bookmaker implied (Shin + proportional devig) + fractional Kelly (per-match AND the H12 same-day cap) + the `predictions` write — reusing M1a's `assembly`/`splits`/`base_learners`/`feature_set` unchanged. M1b also owns: the REAL H1 noise injection (replacing the `apply_noise` stub + resolving the hindcast-row sigma selection), the categorical train/predict code-consistency pin (shared categories), and the feature_hash dtype-drift question (§M21 note). Still deferred: §M3 `wind_serve_risk`/`altitude_serve_boost` interactions + the forecast-vintage `created_at < as_of` weather filter (§M15); the date-granular rank PIT (§M11); the serve/return run-level `bulk_read_failures` metric (§M18 / Codex R6b M2 — Monitor-agent owned); `odds_drift_to_close` (the per-tournament `|line_movement|` aggregate, §M19 — deferred-NULL in v1, needs tournament-level odds plumbing); and the sequential multi-agent loop under one shared `run_id` that makes the §L12 precondition gate fire end-to-end, plus the DI adapter-factory + cron glue (thin deferred layer, §5).
