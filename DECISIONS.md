...
Next session resumes at the **Briefing Agent** (`agents/briefing/`): Claude API summarization + RAG over prior human-edited notes (`briefing.rag`) + the edge/Kelly email render (with the C13 `kelly_disclaimer`), consuming the `predictions` rows M1b now writes. **Orchestrator wiring** is also still open (thin, deferred): the DI adapter-factory construction + the cron shim invoking `DailyPipeline.run_once()`, plus the sequential multi-agent loop under one shared `run_id` that makes the §L12 precondition gate fire end-to-end (DataAgent → ResearchAgent(prediction) → ModelingAgent(prediction) → Briefing). Note the daily pipeline runs the ModelingAgent in **prediction** mode; **training** is a separate, less-frequent job that must run first to populate an active model (M1b's `no_active_model` / `feature_set_mismatch` gates fail-fast otherwise). Still deferred (carry-forward): §M3 `wind_serve_risk`/`altitude_serve_boost` interactions + the forecast-vintage `created_at < as_of` weather filter (§M15); the date-granular rank PIT (§M11); the run-level `bulk_read_failures` metric (§M18 / Codex R6b M2 — Monitor-agent owned); `odds_drift_to_close` (the per-tournament `|line_movement|` aggregate, §M19 — deferred-NULL in v1); the `feature_hash` dtype-drift detection (§M21 note — Monitor/PSI concern); and the optimistic `1/implied` fair-odds basis in the Kelly-ROI backtest metric (`metrics.py` — a diagnostic, not used for live staking; real vigged-odds ROI is a future refinement).
