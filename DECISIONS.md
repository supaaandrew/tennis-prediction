...
└── tests/
    ...
    ├── unit/                  # 1209 tests, all green locally
    ...
    │   ├── test_composition.py # §S7 — builder agent-set+modes; missing-secret raises (dummy env + injected session, no network)
...
migration 013), the bootstrap contract (§S6), and the §S8 exit codes. Next free
single letter (§P/§R skipped, §O is OWM); like §N/§Q the S-rows share the `## M.`
decision table.
