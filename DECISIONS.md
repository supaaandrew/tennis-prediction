...
Next session resumes at **R7** (`agents/research/features/`): fatigue + market-signal extractors, appended to the R6a §M4 extractor-factory registry (additive — no `agent.py` change). Still deferred: §M3 `wind_serve_risk`/`altitude_serve_boost` interactions + the forecast-vintage `created_at < as_of` weather filter (§M15); the date-granular rank PIT (§M11); the serve/return run-level `bulk_read_failures` metric (§M18 / Codex R6b M2 — Monitor-agent owned); and the sequential multi-agent loop under one shared `run_id` that makes the §L12 precondition gate fire end-to-end, plus the DI adapter-factory + cron glue (thin deferred layer, §5).
