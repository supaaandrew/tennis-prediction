...
Next session resumes at the **Monitor Agent** (`agents/monitor/`, AGENTS.md A13 — runs post-briefing **regardless** of upstream status; ECE/PSI/ROI over `config.monitor.windows_days` into `pipeline_runs.metrics`) and/or the **Orchestrator wiring** (still open, thin): the DI adapter-factory construction + the cron shim invoking `DailyPipeline.run_once()`, plus the sequential multi-agent loop under one shared `run_id` that makes the §L12 precondition gate fire end-to-end (DataAgent → ResearchAgent(prediction) → ModelingAgent(prediction) → **BriefingAgent**, now built B1). Note the daily pipeline runs the ModelingAgent in **prediction** mode; **training** is a separate, less-frequent job that must run first to populate an active model (M1b's `no_active_model` / `feature_set_mismatch` gates fail-fast otherwise). Still deferred (carry-forward): the **email-delivery idempotency / outbox** (§N5 — owner: the orchestrator-wiring session); **RAG** over human-edited notes (§N1 — `briefing.rag` present but not consumed; `storage/qdrant/` unbuilt, §E1); §M3 `wind_serve_risk`/`altitude_serve_boost` interactions + the forecast-vintage `created_at < as_of` weather filter (§M15); the date-granular rank PIT (§M11); the run-level `bulk_read_failures` metric (§M18 / Codex R6b M2 — Monitor-agent owned); `odds_drift_to_close` (the per-tournament `|line_movement|` aggregate, §M19 — deferred-NULL in v1); the `feature_hash` dtype-drift detection (§M21 note — Monitor/PSI concern); and the optimistic `1/implied` fair-odds basis in the Kelly-ROI backtest metric (`metrics.py` — a diagnostic, not used for live staking; real vigged-odds ROI is a future refinement).
