...
│       │   ├── __init__.py
│       │   ├── agent.py       # B1 — BriefingAgent(Agent): precondition modeling; §N3 slate window; per-side Shin→prop edge classify; degrade-safe LLM narrate; render; SMTP send; §N4 status
...
Next session resumes at the **Orchestrator wiring** (still open, thin): the DI adapter-factory construction + the cron shim invoking `DailyPipeline.run_once()`, plus the sequential multi-agent loop under one shared `run_id` that makes the §L12 precondition gate fire end-to-end (DataAgent → ResearchAgent(prediction) → ModelingAgent(prediction) → BriefingAgent → **MonitorAgent**, now all built). Note the daily pipeline runs the ModelingAgent in **prediction** mode; **training** is a separate, less-frequent job that must run first to populate an active model (M1b's `no_active_model` / `feature_set_mismatch` gates fail-fast otherwise). The Orchestrator-wiring session also owns **§N5 email-delivery idempotency / outbox**. Still deferred (carry-forward): **RAG** over human-edited notes (§N1 — `briefing.rag` present but not consumed; `storage/qdrant/` unbuilt, §E1); §M3 `wind_serve_risk`/`altitude_serve_boost` interactions + the forecast-vintage `created_at < as_of` weather filter (§M15); the date-granular rank PIT (§M11); `odds_drift_to_close` (the per-tournament `|line_movement|` aggregate, §M19 — deferred-NULL in v1); the run-level `bulk_read_failures` metric (§M18 / §Q8 — Monitor-owned, NOT built this session); `feature_hash` dtype-drift / feature-input PSI (§M21 / §Q8 — Monitor-owned, deferred); the warn/alert threshold CLASSIFICATION of ECE/PSI (`config.monitor.calibration`/`drift` — §Q stores raw metrics, downstream alerting classifies); and the optimistic no-vig `1/implied` ROI basis (§Q2 — diagnostic, real vigged-odds ROI a future refinement).
