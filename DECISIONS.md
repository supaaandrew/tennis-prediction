...
│   └── sync_sackmann_mirror.py # first-live — git clone/pull of the Sackmann mirror + dir-mtime refresh (§C5); 900s timeout, missing-git→GitError (Codex M)
├── ops/                       # deployment glue (env-specific, §S7) — NOT pipeline code
│   ├── alembic.ini            # %(here)s-anchored script_location (runs from any CWD)
│   ├── lib.ps1                # first-live — Import-DotEnv (.env loader), Sync-SackmannMirror, Invoke-Tennis
│   ├── run_train.ps1          # first-live — wrapper: load env → sync mirror → `python -m tennis train`
│   ├── run_daily.ps1          # first-live — wrapper: load env → sync mirror → `python -m tennis run` (06:30 UTC cron)
│   ├── tennis-run.xml         # first-live — Windows Task Scheduler def (06:30 UTC; NOT auto-registered, §S7)
│   └── README.md              # first-live — deployment runbook (bootstrap §S6, long resumable backfill, scraper Cloudflare block)
├── migrations/
│   ├── env.py, script.py.mako
│   └── versions/001..009 + 011..014_*.py # see §3, §H (no 010); 013 = briefing_deliveries (§S5); 014 = matches.matchstat_id (§T10)
├── src/tennis/
...
│   │   ├── weather_uncertainty.py # §M15 forecast band logic (shared by conditions extractor + M1b noise; M1b — avoids models→agents import)
│   │   └── di.py              # Container (singleton + factory scopes)
│   ├── storage/
│   │   └── postgres/          # P1+P2 — rows, protocols, ORM, session, impls
│   │       ├── rows.py        # 19 frozen Row DTOs (zero SQLAlchemy; +BriefingDeliveryRow §S5)
│   │       ├── repositories.py# 19 @runtime_checkable Protocols (zero SQLAlchemy; +BriefingDeliveryRepository §S5)
│   │       ├── models.py      # SQLAlchemy 2.0 declarative ORM (+BriefingDelivery §S5)
│   │       ├── session.py     # PostgresSessionFactory (SET TIME ZONE 'UTC')
│   │       └── impl.py        # 19 concrete repositories (+BriefingDeliveryRepositoryImpl §S5)
│   ├── adapters/
│   │   ├── __init__.py
│   │   ├── http_client.py     # §S7 — HttpxClient: concrete core.contracts.HttpClient (shared transport)
...
│   │   ├── atp_scraper/       # P6 — ATP website (atptour.com) scraper adapter (DEPRECATED — Cloudflare-blocked; retained as §K6 parser-regression baseline)
│   │   │   ├── __init__.py
...
│   │   └── matchstat/         # §T — canonical daily slate source (RapidAPI tennis-api-atp-wta-itf)
│   │       ├── __init__.py    # package exports (slate adapter, client, 4 sidecars)
│   │       ├── client.py      # §T9 two-header auth, §T6 quota counter + lookup cache (persisted), §T8 paginate_all, error mapping (MatchstatAuthError/RateLimit/UpstreamUnavailable)
│   │       ├── models.py      # §T7 pydantic v2 DTOs with string-typed field validators (Decimal odds, verbatim+numeric seed, int best_of, date birthday)
│   │       ├── parser.py      # §T3 player1 convention split (parse_fixtures_page NEVER infers winner; parse_historical_match ALWAYS treats player1 as winner); §K3 Monday-anchor slug derivation
│   │       ├── adapter.py     # §T1 MatchstatScraperAdapter (satisfies ScraperAdapter Protocol); §K1+§K3 match_id; §K2 source_uid; §T10 matchstat_id on both upsert AND update paths (Codex finding 1); §K6/Codex finding 2 run-level zero-parse guard
│   │       └── sidecars.py    # §T11/§T12/§T13/§T14 — VenueGeocoder, RankingsSeeder, SearchEnricher, H2hStatsClient (built; consumers wired in §T-2 follow-up)
│   ├── models/                # M1a/M1b — Modeling Agent ML internals (one module per concern)
│   │   ├── __init__.py
│   │   ├── feature_set.py     # M1a — resolve model X (market family + forecast bucket + backtest-only EXCLUDED) + feature_hash (§M20a/b/d, §M21a/b)
│   │   ├── assembly.py        # M1a/M1b — train assembler + M1b prediction assembler (model.feature_set-bound, §M23 pinned cats) + extract_categorical_categories
│   │   ├── splits.py          # M1a — walk-forward CV; tail carved BEFORE folds; both-sides tournament embargo incl. train↔tail seam (§M20c, §M21c/e)
│   │   ├── base_learners.py   # M1a/M1b — XGB/LGBM native-cat + early stop; OOF + per-learner OOF (oof_xgb/oof_lgbm) for the stacker (§M20a, §M21d)
│   │   ├── noise.py           # M1b — H1 apply_noise(): seeded per-bucket Gaussian on the decision-offset bucket, training-only (§M22)
│   │   ├── stacker.py         # M1b — logistic meta-learner over the per-learner OOF matrix; single-class OOF guard (§M21c)
│   │   ├── calibration.py     # M1b — Platt/isotonic calibrator on the held-out tail; empty/small/single-class → degraded passthrough (§M21c, pre-step 5.1)
│   │   ├── edge.py            # M1b — edge = p1_prob_cal − implied under Shin+proportional; C9 NULL when no odds (§M21a)
│   │   ├── kelly.py           # M1b — fractional Kelly; per-match cap + same-day pro-rata cap; Shin→proportional fallback; NULL/0.0 no-bet (§M24/H12)
│   │   ├── metrics.py         # M1b — ECE + Kelly-ROI backtest (secondary training metrics; pre-step 6.2)
│   │   └── artifacts.py       # M1a/M1b — joblib persistence + µs version mint; TrainedModel carries stacker+calibrator+pinned cats (§M20d, §M23)
│   ├── agents/
│       ├── __init__.py
│       ├── briefing/          # B1 — Briefing Agent (predictions → narrative → edge/Kelly email)
│       │   ├── __init__.py
│       │   ├── agent.py       # B1 — BriefingAgent(Agent): precondition modeling; §N3 slate window; §S5 delivery-idempotency guard (get-before-send / best-effort record-after-send); per-side Shin→prop edge classify; degrade-safe LLM narrate; render; SMTP send; §N4 status
│       │   ├── render.py      # B1 — pure email render: C13 disclaimer per Kelly line, C9 no-market, §M24 NULL-vs-0.0, closing fields never rendered (§M19/§15.4), subject_template
│       │   ├── llm_client.py  # B1 — LlmClient Protocol (§N2) + AnthropicLlmClient (env key, redact_text on error)
│       │   └── email_client.py # B1 — EmailClient Protocol (§N2) + SmtpEmailClient (env creds/recipients, STARTTLS, redact_text §L10)
│       ├── data/              # P7 — DataAgent (daily ingest orchestrator)
│       │   ├── __init__.py
│       │   └── agent.py       # DataAgent(Agent): §J2 order, fault isolation, §L11 venue geocode pass, §L
│       ├── modeling/          # M1a/M1b — Modeling Agent (train → registry; predict → predictions)
│       │   ├── __init__.py
│       │   └── agent.py       # M1a/M1b — ModelingAgent(Agent): mode flag; training=assemble→CV→stack→calibrate→register+activate; prediction=load active→score→edge→Kelly→predictions (per-match isolation, §M19 NULLs, §M22-24)
│       ├── monitor/           # §Q — Monitor Agent (post-pipeline ECE/PSI/ROI observability, A13; NOT in the 4-agent prediction chain)
│       │   ├── __init__.py
│       │   └── agent.py       # §Q — MonitorAgent(Agent): preconditions=() (A13/§Q7); per-window ECE/PSI/ROI over config.monitor.windows_days; realized-outcome join via MatchRepository.list_for_ids (§Q1); rolling-prev-window PSI, next-midnight UTC anchor (§Q4); §Q6 envelope in AgentResult.metrics; monitor_db_error fatal (§Q5)
│       ├── orchestrator/      # P7 + §S4 — pipeline_runs lifecycle owner
│       │   ├── __init__.py
│       │   └── pipeline.py    # §S4 — DailyPipeline(agents): multi-agent chain under ONE run_id; per-agent precondition-skip / failed-continue (Monitor always); aggregate status
...
│   ├── __main__.py            # §S — `python -m tennis` → cli.app
│   ├── cli.py                 # §S2/§S3/§S8 — typer app: `train` / `run` + exit codes
│   ├── composition.py         # §S7/§T1 — build_training_chain / build_daily_chain (DI: AppConfig + env → adapters/clients/agents); shared MatchstatClient; matchstat factories; narrow MissingEnvironmentError catch (Codex finding 4)
│   └── utils/                 # §T4 — cross-source utility modules
│       ├── __init__.py        # re-exports ParsedScore, ScoreParseError, parse_score
│       └── score_parser.py    # canonical score-string parser (both Sackmann + matchstat); complete-sets-only counting; retirement/walkover/tiebreak detection
└── tests/
    ├── conftest.py            # frozen_clock, repo_root, config_path
    ├── unit/                  # 1293 tests, all green locally (was 1221 before §T)
    ...
    │   ├── storage/{test_rows,test_repositories,test_session,test_impl}.py # +BriefingDeliveryRepository (§S5)
    │   ├── test_cli.py        # §S2/§S3/§S8 — CLI command→builder mapping + exit-code rule (mocked, no chain built)
    │   ├── test_composition.py # §S7 — builder agent-set+modes; missing-secret raises; §S6a scraper_required + training-scoped env validation
    │   ├── test_sync_sackmann_mirror.py # first-live — sync script: clone/pull/pin branching, mtime refresh, branch-field lock, GitError (timeout/missing-git, Codex M)
    ...
    │   ├── adapters/atp_scraper/{conftest,test_client,test_parser,test_adapter}.py # 53 (P6 — retained as §K6 regression baseline; deprecated slate)
    │   ├── adapters/matchstat/{__init__,test_client,test_parser,test_adapter}.py # 44 (§T — client: §T9 auth headers + 401/403/429/5xx mapping + §T6 quota counter/persistence/stale-month reset + lookup-cache fresh/stale/bad-cursor/read-fail/naive-tz [Codex finding 5]; parser: §T3 fixture+historical winner split, §T7 string-typed odds/seed/best_of, §C3 tier-drop; adapter: happy path + §T10 matchstat_id on update path [Codex finding 1] + §K6 run-level zero-parse guard [Codex finding 2])
    │   ├── utils/test_score_parser.py # 25 (§T4 — straight/tiebreak/deciding/retirement/walkover/malformed/contract)
    │   ├── adapters/test_http_client.py # §S7 — HttpxClient get/post via httpx.MockTransport (no network)
    │   ├── models/{test_feature_set,test_assembly,test_splits,test_base_learners,test_noise,test_artifacts}.py # M1a (+conftest)
    │   ├── models/{test_stacker,test_calibration,test_edge,test_kelly,test_metrics}.py # M1b
    │   └── agents/
    │       ├── briefing/{conftest,test_render,test_agent,test_clients}.py # B1 — render (C13/C9/§M19), agent (no-model/no-qualifying/NULL-only-slate/partial/SMTP-fail/LLM-degrade/§N3/§L2/active-version), _classify per-side (Codex F1), clients (redaction)
    │       ├── monitor/{conftest,test_agent}.py # §Q — Monitor: preconditions=() (A13), per-window ECE/PSI/ROI envelope (§Q6), pending excluded-but-counted + winner-not-in-match (§Q1), zero-settled→partial, rolling PSI + no-prior→None (§Q4), active-version scope, db-error→failed + envelope (§Q5/F1), bulk-read N+1 perf guard (F2), naive/non-UTC as_of (F3)
    │       ├── data/test_agent.py            # 20 (P7 — DataAgent control flow + §L11 geocode pass; §S6a scraper-optional: tolerate operational 403, still surface unexpected exceptions)
    │       ├── modeling/{conftest,test_agent}.py # M1a — Protocol, precondition, ≤1-active flip, WO/invalid-winner drop, OOF gate, redaction
    │       ├── orchestrator/test_pipeline.py # P7 + R6a + §S4 — lifecycle + per-agent gate; multi-agent chain (one run_id/lock/sweep), data-fail→skip-but-Monitor-runs, run()-raise→failed-continue, §S6 bootstrap sequence
...
All four pipeline agents + the Monitor are built and wired. The §S orchestrator-
wiring session shipped the multi-agent sequential loop (`DailyPipeline(agents)`,
§S4 — all stages under one `run_id` so the §L12 precondition gate fires end-to-
end), the composition root (`composition.py` + the `HttpxClient` transport, §S7),
the `train`/`run` CLI (`cli.py` + `__main__.py`, §S2/§S3), and the §N5 resolution
(`briefing_deliveries`, §S5). **Remaining (out of repo scope):** `storage/qdrant/`
(N1 RAG, deferred) and the OS scheduler (cron/systemd at 06:30 UTC — deployment
config, §S7), plus the §M3/§M18/§Q8 deferrals tracked in their own rows.
...
Locks for the Research Agent feature families (R2–R7) **and** the Modeling
Agent (M1 onward — §M20+). The `§M` letter is the shared decision namespace,
not a Research-only marker. Letter `M` (`G` is "intentionally skipped"; `M` is
the next free letter that does not collide with build-phase `P#`, session
`R#`, or validator-rule `R#` labels). New feature rows are mirrored into
§15.5; new config keys live under `features.*` / `modeling.*` (never
hardcoded).
...
| M20 | **Modeling Agent foundational locks (M1, `agents/modeling/agent.py` + `models/`).** Four locks settled BEFORE the M1 build (the modeling-side mirror of the §M Research locks). **(a) Categorical encoding — native categorical, NO one-hot.** Every `feature_specs` key with `dtype="cat"` is fed as a pandas `category` dtype with XGBoost `enable_categorical=True` and LightGBM `categorical_feature=`; there is no one-hot or ordinal expansion anywhere in the pipeline. The model's input column set therefore equals the active feature-key set (no synthetic dummy columns). **(b) Backtest-only denylist is a STRUCTURAL module constant, not config.** `_BACKTEST_ONLY_KEYS = frozenset({"p1_implied_pinnacle_closing", "line_movement_p1", "odds_drift_to_close"})` — same code-side-constant pattern as `_CRITICAL_FEATURE_KEYS` (§M8), NOT a `config.modeling.*` knob. These keys are dropped from the LIVE feature set so a column that is structurally NULL at a live decision instant (§M19 / §15.4) can never be trained on, persisted into a live row, or fed to the live scorer (the modeling-side mirror of §M19). **(c) Tournament embargo drops whole tournaments from BOTH sides of the fold boundary.** With `embargo_by="tournament_id"`, any tournament straddling a walk-forward fold boundary is excluded from BOTH the train fold AND the test fold adjacent to it — never train-side only. No intra-tournament leakage can cross the seam in either direction. **(d) Artifact persistence.** Serialization = `joblib`; storage = a local path under `config.modeling.artifact_dir` (NEW config key, added in M1a); `feature_hash` = SHA-256 hex of the sorted active feature-key list; `version` string (the `model_registry.version` PK) = `"{season_start}-{season_end}-{timestamp_utc}"`. | Native categorical avoids one-hot column explosion and keeps the model's input columns identical between train and predict, so `feature_hash` is meaningful and an unseen category degrades gracefully instead of silently shifting column positions. A structural denylist (not config) follows §M8's critical-key precedent: the backtest-only set is a correctness invariant, not a tunable, so a config edit can never silently widen/narrow it and a `*_closing` look-ahead leak can never reach the live scorer. Embargoing straddling tournaments on BOTH sides is the only rule that removes leakage in both directions — a train tournament bleeding into an adjacent test tournament AND test-fold knowledge bleeding back into train; a train-side-only embargo leaves the reverse leak open. joblib + a config-driven local dir + SHA-256(sorted keys) + a window+timestamp version are the minimal deterministic choices that satisfy `model_registry`'s NOT NULL `artifact_uri` / `feature_hash` / `version` columns without a blob store in v1 (E-series defers cloud artifact storage). |
| M21 | **Modeling leakage & label-hygiene locks (M1, approved pre-plan from the M1 leakage audit).** Six locks. **(a) Market family excluded from the model feature set.** The model's X (train AND live) = active `feature_specs` keys EXCLUDING the entire `market` family AND `forecast_uncertainty_bucket`; the decision-time implied keys (`p1_implied_pinnacle_decision`, `p1_implied_proportional_decision`, `consensus_implied_p1`) are read directly from the matrix payload for the EDGE step ONLY, never as model inputs. Broadens §M20(b): ALL 8 market keys are non-features, not just the 3 backtest-only ones. **(b) `forecast_uncertainty_bucket` is a noise-injection CONTROL signal, not a model feature.** It is NULL on every hindcast training row and non-NULL only at prediction, so as an X column it is a pure train/predict mismatch carrying zero training signal; it drives the H1 per-row noise sigma and is excluded from X. **(c) Calibration tail carved BEFORE CV.** The last `modeling.calibration.tail_days` of the assembled `for_training` data is removed FIRST; walk-forward CV, the final base-learner fits, and the stacker fit run ONLY on the older remainder; the calibrator is fit on the tail. The tail never enters a CV fold (no OOF→calibration leak — the classic nested-CV mistake). **(d) Early-stopping eval set is a train sub-split.** Each fold's base learners (`early_stopping_rounds=50`) early-stop on a chronological sub-split of THAT fold's TRAIN portion, NEVER the held-out OOF/test fold, so the stopping signal cannot leak into the OOF predictions that train the stacker. **(e) Tournament embargo also covers the train↔tail seam.** The §M20(c) both-sides tournament embargo applies at the calibration-tail boundary too: a tournament straddling the train/tail cut is dropped from both sides. **(f) Training label hygiene — drop walkovers, keep retirements.** The modeling data-assembly step excludes `matches.walkover=True` rows from the training label set (administrative non-contest → pure label noise) and KEEPS `matches.retired=True` rows (real play, settled W/L — mirrors Elo §M10). `for_training` itself is UNCHANGED (C4 status pin); the filter lives in the modeling assembly, not the repo. **Config guards:** `modeling.splits.embargo_by` is hard-validated (reject unknown/empty — never silently skip embargo); `modeling.calibration.tail_days` must be `> 0`; `features.weather.inject_forecast_noise=false` is permitted but logged as a known weather hindcast/forecast-skew risk (H1). **`feature_hash` dtype-drift (DEFERRED — M1a adversarial review):** `feature_hash` is SHA-256 of the sorted feature KEY list ONLY (per §M20d), so a dtype-only catalog change (e.g. `int`→`float`, a `cat` reclassification) keeps an identical hash and the hash alone does NOT detect train/serve dtype drift. Accepted for v1: the §M7 catalog-lockstep + the R2 `FeatureMatrixValidator` already reject dtype/catalog drift at the Research seam before Modeling reads, and M1b's live scorer must pin categorical codes regardless — so dtype-drift detection (folding `key:dtype` into the hash, or a Monitor PSI check) is an M1b/Monitor concern, not an M1a change. | Decision-time odds are pre-match (not temporal leakage) but are the single sharpest signal — a model that ingests them reproduces the market and the edge `p1_prob_cal − implied` collapses to noise; excluding the market family forces an independent estimate the edge step can meaningfully diff. `forecast_uncertainty_bucket` carries no training signal (always NULL pre-inference) and exists only to size H1 noise, so it is a control input, not a feature. Carving the tail before CV is the only ordering that keeps the calibrator's rows out of every stacker-training fold. Early-stopping on the OOF/test fold lets the base learner peek at the rows whose predictions train the stacker, so the eval set must come from inside the fold's own train block. The tail boundary is just another fold seam, so the §M20(c) embargo extends to it. Walkovers are non-contests no skill feature can predict (pure calibration noise); retirements follow real play with a settled result, so they stay (consistent with Elo §M10 and C14). The config guards stop a silent edit from disabling embargo or degenerating the tail, and surface the weather-skew tradeoff when noise is off. |
| M22 | **H1 training-noise sigma rule (M1b, `models/noise.py`).** Training rows are hindcast, so `forecast_uncertainty_bucket` is NULL → there is **no per-row sigma**. The injection sizes its Gaussian noise from the **single bucket that corresponds to `decision_timing.live_decision_offset_hours`** — resolved through the same §M15 half-open bands (`0≤h<6`→low, `6≤h<24`→medium, `h≥24`→high), so the locked v1 offset of 24h → **`"high"`**. Sigmas are read per weather COLUMN from `config.features.weather.noise_sigma_by_bucket[bucket]` (column→sub-key map: `temp_c_decision`→`temp_c`, `humidity_pct_decision`→`humidity_pct`, `wind_speed_ms_decision`→`wind_speed_ms`); a weather column with no matching sub-key is left untouched. Noise is `N(0, sigma)` added to a **COPY** of those columns in the TRAINING loop ONLY (H1 — `feature_matrix` always stores clean values; `apply_noise` is **never** called on the prediction path). The RNG is **seeded** from the NEW `config.modeling.random_seed` (default 42) via `np.random.default_rng`, so each retrain — and the unit suite — is deterministic. `features.weather.inject_forecast_noise=false` is honored as a no-op and logged as the §M21 weather hindcast/forecast-skew risk. NaN cells stay NaN (native missing; noise is added, the value is not invented). **CV runs on the NOISED frame (intentional, Codex M1b):** the noise is applied ONCE to `X_model` and that same noised frame feeds walk-forward CV, the stacker's OOF training, AND the final base-learner refit — NOT clean `X` for CV and noised only for the final fit. This is deliberate: the deployed base learners are trained on the noised remainder, so the stacker must be trained on OOF produced by noised-trained base learners (and the calibrator on the noised tail) to match the deployed distribution. Grading/selecting on clean-`X` CV would train the stacker on clean-trained base-learner behaviour and then deploy it on top of noised-trained base learners — a stacker/base-learner mismatch. Consistent noising across CV, stacker, tail, and final fit also honours H1's "injected immediately before fitting each base learner" (CV folds fit base learners too). | The training feature matrix is hindcast (clean, bucket NULL) but the live scorer reads forecasts whose error grows with horizon; injecting the horizon-appropriate sigma at train time closes that train/serve distribution shift (H1/§M21) without ever persisting noise. With no per-row bucket available on hindcast rows, the decision-offset bucket is the principled, single, config-derived choice — it is exactly the horizon the live model will face at T-`live_decision_offset_hours`. Seeding is required because training noise is otherwise non-reproducible, which would violate the same retrain-determinism concern that motivated H1 in the first place; a config knob (not a magic literal) keeps it tunable and visible. Per-column sigma keyed by the existing `noise_sigma_by_bucket` map avoids inventing a parallel config surface. |
| M23 | **Categorical code pinning (M1b, `models/{artifacts,assembly}.py`).** `TrainedModel` stores `categorical_categories: Mapping[str, tuple[...]]` — the exact `category` levels of each categorical column as seen in the TRAINING frame — captured at fit time. At predict time the prediction assembler applies `pd.CategoricalDtype(categories=training_cats, ordered=False)` to each categorical column **before** the frame reaches the base learners, so the integer codes XGBoost/LightGBM consume are **identical** between train and serve. An **unseen category → NaN** (native missing, the §M20a contract), never a crash and never a silent code remap. The prediction assembler is driven by **`model.feature_set` (the loaded artifact's contract), NOT a freshly resolved `resolve_model_feature_set(active_specs)`** — so a catalog change between training and prediction cannot shift columns out from under a serialized model; the fresh resolve is training-mode only. | `astype("category")` assigns codes per-DataFrame, so a prediction frame built independently would map the same string to a different integer code (or omit/extend levels), silently corrupting every categorical split in the trees — the single highest train/serve bug risk (M1a gotcha #1). Pinning the training levels and re-applying them is the only way to guarantee code identity; routing unseen categories to native NaN degrades gracefully (the value is missing, not wrong) instead of raising at serve time. Sourcing the feature set from the artifact (not the live catalog) makes the model self-describing and immune to catalog drift between train and predict — the storage-side complement to §M20d's `feature_hash`. |
| M24 | **Kelly same-day cap mechanics (M1b, `models/kelly.py`).** **(a) "Same-day" key = `predicted_at.date()`** (the UTC decision instant's calendar date), NOT the match date — the cap governs capital committed in one decisioning run. **(b) Both caps ALWAYS apply (H12).** Per-match: each raw fractional-Kelly stake is first clamped to `config.modeling.kelly.max_exposure_pct`. Same-day: if `Σ(clamped stakes for the day) > config.modeling.kelly.max_total_exposure_pct`, every stake is scaled by `max_total_exposure_pct / Σ` (pro-rata) so the day's total equals the cap; if the sum is already within the cap, no scaling. **(c) Sizing basis = Shin, fallback proportional.** The stake uses the Shin edge + Shin decision-implied odds; when the Shin implied prob is NULL/degenerate it falls back to the proportional implied (mirrors `config.features.market.devig_method_primary→fallback`). **(d) No-bet semantics:** `kelly_fraction_*` is **NULL when no usable odds exist** (C9 — cannot size) and **0.0 when odds exist but `edge < config.modeling.edge.min_edge_to_bet`** (sized, deliberately not bet) — the two cases are distinguishable on the stored row. | A per-match cap alone permits N qualifying bets × `max_exposure_pct` to commit an unbounded fraction of bankroll on a heavy slate (H12's exact failure: 10 bets × 2% = 20% with no circuit breaker); the same-day total cap is the missing circuit breaker. Keying it on `predicted_at.date()` ties the limit to a decisioning run (all of a morning briefing's bets), which is what bankroll exposure actually means. Pro-rata scale-down preserves the relative Kelly ranking while honoring the ceiling. Shin is the project's primary devig (the sharpest implied estimate); the proportional fallback keeps a stake sizeable when Shin degenerates rather than dropping the bet. Distinguishing NULL (no odds) from 0.0 (no edge) keeps the row honest — the briefing can tell "we couldn't price it" from "we priced it and passed" (C9). |
| M25 | **M1b prediction-mode hardening (Codex M1b adversarial review, `agents/modeling/agent.py`).** Three fail-safe locks on the scoring path. **(a) Feature-set contract guard.** Before scoring, the agent asserts `model_registry.active().feature_set == config.features.feature_set`; on mismatch it returns `failed` with the NEW fatal code `feature_set_mismatch` (added to `pipeline._FATAL_CODES`) and ZERO writes, and the `feature_matrix` read is keyed on the ACTIVE MODEL's family — never blindly on live config. A config-side feature-set rename after training therefore can never silently score the wrong payload family (or mass-dead-letter an empty slate). **(b) Per-match scoring isolation.** The base-learner → stacker → calibrator predict is done ONE row at a time INSIDE the per-match `try/except` (results are identical to a batch predict; cheap on a daily slate), so a single row that makes `predict` raise is dead-lettered and the rest of the slate still scores — honouring the §L2 per-match-isolation contract on the scoring path, not just the upsert path. **(c) Non-finite probability guard.** A non-finite `p1_prob_raw`/`p1_prob_cal` (e.g. a stacker/calibrator emitting NaN) is rejected BEFORE upsert and dead-lettered, rather than relying on the `predictions` `CHECK (… BETWEEN 0 AND 1)` to bounce it at the DB. **(d) Base-only servability guard.** An active artifact that predates M1b stacking (`stacker`/`calibrator` is None) returns `failed` (`no_active_model`) rather than crashing on a None stacker. | Codex flagged three production-break paths: a feature-set config drift silently scoring the wrong family; a single bad row aborting the WHOLE slate because the batch predict sat OUTSIDE the per-match guard; and a NaN probability reaching the row. Each fix converts a silent-corruption / whole-run-abort into a loud, isolated, recoverable outcome (fail-fast for systemic mismatch; dead-letter + `partial` for a single bad row). Per-row predict is row-independent, so isolation costs only a negligible per-call overhead on the small daily slate while removing the batch single-point-of-failure. The servability guard closes the M1a→M1b transition window where a base-only model is still active before the first M1b training run supersedes it. |
| N1 | **RAG deferred (Briefing Agent narrative, B1, `agents/briefing/`).** v1 Briefing summarizes the day's `predictions` **WITHOUT** retrieval. `config.briefing.rag` (`top_k`/`filters`/`prefer_human_edited_notes`) is present but **NOT consumed** — no notes/embeddings store exists (`storage/qdrant/` is unbuilt per §5). The narrative is generated from the surfaced slate + per-match `feature_matrix` payloads only. RAG is a documented post-v1 follow-up that touches ONLY the prompt-assembly stage, never the data pipeline. | Building a Qdrant corpus + Files API + cross-session memory durability exceeds the B1 scope; documenting the defer-with-a-single-owner (like §M3/§M15/§M19) keeps B1 bounded rather than half-building a retrieval layer. |
| N2 | **`LlmClient` + `EmailClient` are `@runtime_checkable` Protocols injected at ctor (B1, `agents/briefing/{llm_client,email_client}.py`).** Both mirror the `core/contracts.py` `HttpClient` precedent — the agent depends on the abstract shape, not `anthropic`/`smtplib`. Unit tests **ALWAYS** inject mocks; the suite **NEVER** calls the real Anthropic API or sends real email (no network, <5s). The concrete `AnthropicLlmClient`/`SmtpEmailClient` read secrets from env (`llm.api_key_env`, `smtp.host_env`/`user_env`/`password_env`, `email.recipients_env`) via `read_required_env` at construction — never hardcoded. SDK/HTTP/SMTP errors are wrapped in a typed `BriefingLlmError`/`BriefingEmailError` whose cause is `redact_text`'d (§L10). | Network/SMTP in tests is slow, flaky, and risks leaking cost/secrets; the Protocol seam keeps B1 testable and the client swappable, exactly like every adapter depends on `HttpClient` not `httpx`. |
| N3 | **Slate window (B1).** `since = ctx.as_of.date()` at **midnight UTC**; `until = since + 1 day`. Passed to `PredictionRepository.list_for_window(model_version=active.version, since, until)` (filters `predicted_at` in the half-open `[since, until)`). Covers every prediction written for the decisioning day. | The daily run writes predictions at `ctx.as_of` (~06:30 UTC); a midnight-to-midnight-UTC window on the `as_of` date captures the whole day's slate deterministically, independent of the exact run time. |
| N4 | **Status semantics (B1, AGENTS.md → BriefingAgent).** `succeeded` = email sent; `partial` = email sent but ≥1 surfaced row had NULL edges (C9 "no market"); `failed` = no active model **OR** no qualifying predictions **OR** SMTP failure. **"Qualifying"** = ≥1 row whose primary (Shin→proportional fallback) edge is `>= config.modeling.edge.min_edge_to_log`. **Surfaced set** = qualifying rows **∪** NULL-edge "no market" rows (priced-but-below-threshold rows are dropped). A **NULL-edge-only slate** has zero qualifying rows → `failed`, **ZERO email** (AGENTS.md literal "no predictions above `min_edge_to_log`"); NULL rows are surfaced as context WITHIN an email a real edge triggers, never dropped (C9), never a send trigger on their own. An **LLM failure is NOT a `failed` trigger** — the email still sends without the narrative (`narrative_degraded=True` in metrics), status decided by the edge rule. The codes `no_qualifying_predictions`/`smtp_send_failed` are added to `pipeline._FATAL_CODES` so `_map_status` returns `failed` (not `partial`). | AGENTS.md fixes exactly three `failed` triggers; C9 forbids hiding a prediction we brief on. The NULL-only-slate edge case (where AGENTS.md and C9 diverge) is resolved to `failed`/no-email — nothing actionable to send — so `partial` strictly means "edges sent + some no-market context", keeping both ground-truth docs consistent. LLM degradation is non-fatal because the edge/Kelly table is the email's load-bearing content, not the prose. |
| N5 | **Email-delivery idempotency DEFERRED v1 (Briefing Agent, B1).** B1 sends via `EmailClient.send` with **no durable delivery guard** — a manual re-run, or a process crash between a successful `send` and the `pipeline_runs` status write, can re-send the day's briefing. Concurrent double-send is ALREADY prevented by the orchestrator's cluster-wide advisory lock (§L7 "no overlap") + orphan-sweep; the residual is the narrow crash-after-send / deliberate-re-run window. A durable fix — a `briefing_deliveries` outbox table (unique on `briefing_day_utc + model_version`, or `run_id`), insert-if-absent before send + persisted terminal delivery state so a retry detects prior success and skips — requires a NEW migration AND its own locked decision, exceeding B1's scope (spec.md outputs are "email sent; metrics updated"; AGENTS.md has no idempotency clause). Owner: the deferred **orchestrator-wiring** session (the DI + cron shim that invokes `DailyPipeline.run_once()`). | Mirrors the §M3/§M15/§M19 defer-with-a-single-owner discipline — bolting a delivery-outbox migration onto B1 would half-build a cross-cutting concern. The §L7 advisory lock already removes the concurrent-run failure mode, so the residual risk is low and bounded; documenting it with a named owner beats an unplanned schema change. (Codex B1 adversarial review rated this HIGH; reassessed to a documented v1 limitation given the existing §L7 lock + out-of-scope migration.) **RESOLVED in §S5** — the `briefing_deliveries` outbox (migration 013, `UNIQUE(briefing_day_utc, model_version)`) + get-before-send / best-effort record-after-send guard is now built and wired into the BriefingAgent. |
| Q1 | **Realized-outcome join (Monitor, §Q, `agents/monitor/agent.py`).** A prediction is scored by `match_id → MatchRepository.list_for_ids` (bulk; Q-impl below); label `y=1` iff `matches.status=='final'` AND `winner_id==p1_id`, `y=0` iff final AND `winner_id==p2_id`. Not-yet-final / no-winner / `winner_id ∉ {p1_id,p2_id}` (data corruption) → **pending**: excluded from ECE/ROI, **counted** in `n_pending`, never guessed as a p2 win. Scoped to `ModelRegistryRepository.active().version` (mirrors Briefing §N4). | ECE/ROI need realized labels; a live slate's matches have no winner yet so pending rows are tracked not failed, and a corrupt winner must not silently score as a loss. |
| Q2 | **Live ROI basis (§Q).** Reuse `models.metrics.roi_kelly_backtest` per window: `probs=p1_prob_cal`, `y_true`=Q1 label, `implied_p1=p1_implied_decision` (the Shin-primary/proportional-fallback decision implied on `PredictionRow`); settled at the fair `1/implied` decimal odds. **Accepted optimism (documented):** `1/implied` is a no-vig fair price → live ROI is systematically optimistic vs a real vigged payout, but it uses the ACTUAL decision-time price each bet faced and needs NO separate settled-odds lookup (closing odds are NULL on live rows, §M19/§15.4). True vigged-settlement ROI is a future refinement. | Reuses the existing Kelly-ROI math (DRY); the decision-time implied is the honest, available staking basis; documenting the no-vig optimism keeps the metric honest. |
| Q3 | **ECE (§Q).** Monitor reuses `models.metrics.expected_calibration_error(y_true, probs)` at its default **10** equal-width bins — NO second implementation. Empty/NaN → window metric `None`. Config promotion of the bin count deferred (no `config.monitor` knob). | One ECE across Modeling and Monitor avoids divergence; 10 bins is the established default. |
| Q4 | **PSI definition + rolling reference (§Q, `models/metrics.py`).** New `population_stability_index(actual, expected, *, n_bins=10, epsilon=1e-6)` — equal-width [0,1] bins (same grid as ECE), epsilon-floored fractions so an empty bin yields a finite term (no log(0)/±inf). Subject = the `p1_prob_cal` distribution, **settlement-independent** (ALL predictions in a window, settled+pending). **Reference = the immediately-preceding, equal-width, NON-overlapping window:** with anchor = **NEXT midnight UTC after `as_of.date()`** (so the current window `[anchor−D, anchor)` INCLUDES the run-day's ~06:30 predictions — review H1), reference = `[anchor−2D, anchor−D)`. A reference window with no rows (first-ever run) → NaN → `None`, never a crash. Thresholds in `config.monitor.drift.psi_threshold_warn/alert`. **Limitation:** detects temporal drift, not deviation-from-training (training-distribution PSI deferred — needs a stored training histogram). | A rolling prior-window reference is storage-free and reuses `list_for_window`; output drift is settlement-independent; next-midnight anchoring matches B1's `_slate_window` and avoids dropping the run day; epsilon-flooring keeps empty bins finite. |
| Q5 | **Status semantics + gate (§Q).** `lineage.preconditions=()` (A13) — runs regardless of upstream; the `run_monitor_post_briefing` gate lives in the ORCHESTRATOR, not `run()`. `succeeded` = every window produced all three metrics (ece/psi/roi non-None) + written; `partial` = ≥1 window had too few/zero settled rows or a degraded metric (e.g. no reference window yet → PSI None), OR no active model — codes `monitor_partial` / `monitor_no_active_model`, both **NON-fatal**; `failed` = a DB read failed → `monitor_db_error` (the ONLY Monitor code in `pipeline._FATAL_CODES`). `as_of` is rejected if naive and **normalized to UTC** before all window math (review F3 — a non-UTC `as_of.date()` would shift the anchor a day). `run` NEVER raises for these documented modes. | The Monitor is observability; "no data yet" (zero settled, no model, no reference) is partial not failure (item 3). Only a DB outage is fatal. Empty preconditions are what let the Monitor observe a degraded pipeline. UTC normalization keeps windows deterministic run-to-run. |
| Q6 | **`pipeline_runs.metrics` JSON envelope (§Q).** `{"model_version": str\|null, "as_of": iso8601, "windows": {"<days>": {"ece": float\|null, "psi": float\|null, "roi": float\|null, "n_settled": int, "n_pending": int}}}`. Window keys are the `config.monitor.windows_days` values as strings; a zero/too-few-settled or reference-less window has `null` metrics with truthful counts. **The SAME envelope is returned on EVERY terminal state — including `monitor_db_error` (model_version null, windows {}) and no-active-model** (review F1) — so a downstream reader sees one fixed shape. The agent only RETURNS it in `AgentResult.metrics`; the orchestrator persists it via `update_status(metrics=…)` (no direct run-row write, no `PipelineRunRepository` dep). | A fixed per-window envelope keyed by window width is queryable and honest about gaps (null + counts); emitting it even on the fatal path avoids breaking readers on exactly the failure operators inspect; riding the existing metrics-write path keeps the agent read-only. |
| Q7 | **Monitor is observability, not the prediction chain (§Q).** The prediction chain is the **4** agents Data→Research→Modeling→Briefing; the Monitor is a **5th** `pipeline_runs` row (one shared `run_id`) that runs post-briefing regardless of upstream status (A13). AGENTS.md's "five stages" counts the observability stage; "4-agent prediction chain" counts only the agents that produce the daily prediction/briefing. | Stating it explicitly prevents a future reader from adding the Monitor to the prediction-chain preconditions or treating its `partial`/`failed` as a daily-deliverable failure. |
| Q8 | **Bulk match read + deferred Monitor-owned metrics (§Q).** The Monitor's outcome join uses a NEW `MatchRepository.list_for_ids(*, match_ids) -> Mapping[int, MatchRow]` (Protocol + `MatchRepositoryImpl` + empty-`{}` fast-path, single IN-list, typed `StorageError` on failure) — the bulk dual of `get`, ONE read per window instead of one `get` per prediction (review F2, mirrors the §M18 anti-N+1 discipline). **Deferred (NOT built this session, Monitor-owned):** (a) §M18 run-level `bulk_read_failures` counter (needs a ResearchAgent run-diagnostics channel — extractors expose only `extract() -> Mapping`); (b) §M21 feature-input / `feature_hash` dtype-drift PSI (needs a stored training distribution); (c) warn/alert CLASSIFICATION of ECE/PSI against `config.monitor.calibration`/`drift` thresholds — v1 stores RAW metrics; a downstream alerting layer (or a future Monitor hardening session) classifies them. | A per-prediction `get` over multi-thousand-row windows × 3 windows is an N+1 the §M18 discipline forbids; the bulk dual fixes it cheaply. Each deferred item needs a cross-cutting surface (Research diagnostics channel / training histogram / alerting layer) out of a Monitor-only session (defer-with-a-single-owner, like §M3/§M15/§M19). |
| S1 | **Decision namespace `§S` (orchestrator-wiring session).** Next free single-letter section; `§G`/`§P`/`§R` remain skipped per the §M convention (build-phase `P#`, session/validator `R#`), `§O` is OWM, `§N`/`§Q`/`§S` share the `## M.` table. | A grep of DECISIONS.md confirmed `§S` was unused; reusing the established skip convention keeps the namespace collision-free. |
| S2 | **Training entrypoint `python -m tennis train` (`cli.py`, `composition.build_training_chain`).** Data → Research(`mode="training"`) → Modeling(`mode="training"`) as ONE chain under ONE `run_id` (§S4). NO Briefing/Monitor. Separate from the daily prediction cron — training ingests, builds the `for_training` matrix, and trains+registers+activates a model. | Training and prediction are distinct jobs with different agent sets and cadence; a model must exist before any daily run (§S6). |
| S3 | **Daily prediction entrypoint `python -m tennis run` (`cli.py`, `composition.build_daily_chain`).** Data → Research(`mode="prediction"`) → Modeling(`mode="prediction"`) → Briefing → Monitor as ONE chain under ONE `run_id`. The 06:30 UTC cron. **Monitor is gated by `config.orchestrator.run_monitor_post_briefing`** (§Q5/§Q7/A13): `build_daily_chain` OMITS `monitor_agent()` from the chain when the flag is `False` (the agent's empty preconditions mean the loop would otherwise always run it). | The full daily deliverable is the four-agent prediction chain plus the Monitor observability stage (§Q7), all sharing a `run_id` so preconditions gate correctly. The gate lives at the orchestration/composition layer, never inside `MonitorAgent.run` (§Q7). |
| S4 | **Multi-agent sequential loop (`agents/orchestrator/pipeline.py`).** `DailyPipeline(agents: Sequence[Agent])` runs the chain under ONE `run_id`, ONE §L7 singleton advisory lock, ONE orphan sweep (before the first agent). Per agent: precondition gate → insert running row `(run_id, agent, 1)` → `run` → `_map_status` → `update_status`. A not-met precondition → **SKIP** (NO `pipeline_runs` row — there is no `"skipped"` status; structured warning; loop continues); downstream gated agents skip naturally; empty-precondition agents (Data, Monitor A13) always run. An UNEXPECTED exception escaping `run()` → record that agent `failed` and **continue** (so Monitor still runs), NOT re-raised. §L2 startup guard (sweep+first insert → `PipelineStartupError`) and §L8 terminal-write propagation preserved. `run_once()` returns the AGGREGATE status over the agents that ran (`failed` if any failed, else `partial` if any partial, else `succeeded`), or `None` if the lock was held. | A downstream precondition ("data succeeded for THIS run_id") is only satisfiable once all stages share one `run_id` in a sequential loop — the deferred piece AGENTS.md §3 named. Skip-without-a-row avoids inventing a `"skipped"` RunStatus (type + DB-enum + migration churn). Continuing past a crashed agent honors A13 (Monitor must observe even a degraded pipeline). The aggregate gives the CLI one §S8 exit signal while staying backward-compatible with the single-agent return contract. |
| S5 | **§N5 RESOLVED — email-delivery idempotency (`briefing_deliveries`, migration 013).** Table keyed `UNIQUE(briefing_day_utc, model_version)` (`run_id` audit-only). BriefingAgent: `get(briefing_day_utc=as_of.date(), model_version=active.version)` BEFORE the LLM/render/send — a prior row → `succeeded`, zero email (`already_delivered`); after a confirmed `send`, `record(...)` via INSERT … ON CONFLICT DO NOTHING. The post-send write is **best-effort** (a `record` failure is logged, NOT propagated — must not flip a genuinely-sent briefing to `failed`); the pre-send `get` `StorageError` propagates → `briefing_db_error` (fail-closed: never double-send on an indeterminate DB). The (day, model) key survives across `run_id`s, closing BOTH the deliberate-re-run and the crash-after-status-write windows §N5 named; a new model version legitimately re-briefs the same UTC day. **Accepted residual:** a crash OR a transient DB error in the window between SMTP-OK and `record` may re-send on a later run (irreducible without a distributed txn — the same window §N5 acknowledged). The swallowed `record` failure is logged at **ERROR** as a pageable `briefing_delivery_record_failed` signal so an operator repairs idempotency state before the next run (full outbox-before-send deferred — its own session). | The §L7 lock already prevents CONCURRENT double-send; §S5 adds the durable guard for the manual-re-run / crash-after-send residual. Keying on (day, model) not `run_id` is the only choice that survives a fresh `run_id`; record-after-send keeps the row truthful; best-effort record + fail-closed get bias every failure mode away from a double-send while keeping a real DB outage loud. |
| S6 | **Bootstrap sequence (cold start / no active model).** The daily chain requires an ACTIVE model; `train` must run first. With no active model: Modeling returns `no_active_model` (fatal, in `_FATAL_CODES`) → `failed`; Briefing is SKIPPED (its `modeling succeeded` precondition is unmet, §S4 — no row); Monitor still runs (empty preconditions) and reports `monitor_no_active_model` → `partial` (NON-fatal). The aggregate is `failed` (CLI exit 1). **This is expected, not a bug.** | Documents the cold start so a failed first daily run is not mistaken for a defect; the §S4 skip + existing `_FATAL_CODES` membership already produce exactly this sequence — §S6 locks it as intended behavior with a regression test (`test_bootstrap_no_active_model_sequence`). |
| S7 | **Composition root + secrets (`composition.py`, `adapters/http_client.py`).** ONE module wires the production graph from `AppConfig` + env: `PostgresSessionFactory` → the `impl.py` repositories → the source adapters (each via a `Callable[[Clock, UUID], Adapter]` factory closure) + `AnthropicLlmClient`/`SmtpEmailClient`. A NEW concrete `HttpxClient` (httpx wrapper over the existing `core.contracts.HttpClient` 2-method Protocol) is the shared transport — the adapters/clients/`FilesystemMirrorReader` already existed; only the transport was missing. Secrets read from env (`read_required_env`) at construction; `validate_environment` runs first (fail-fast). Network egress / DB connections happen only when an agent RUNS, so the builders are unit-testable with dummy env + an injected session factory (no network). The OS scheduler (cron/systemd at 06:30 UTC) is deployment config — documented, NOT installed in-repo. | A single composition root keeps wiring out of the agents (which depend only on Protocols) and is the natural home for the one missing piece, a concrete HTTP transport. Lazy network keeps the builders testable. The scheduler is environment-specific and out of repo scope. |
| S8 | **CLI exit codes (`cli.py`).** `0` when every agent that ran ended `succeeded`/`partial`; `1` when any ran agent ended `failed`. Monitor `partial` (e.g. `monitor_no_active_model`) never forces nonzero. A `None` result (singleton lock already held, §L7) is a benign no-op → `0`. `_exit_code(status) = 1 if status == "failed" else 0` over `run_once()`'s §S4 aggregate. | A cron/operator needs one unambiguous signal — did the run produce its deliverable? The aggregate is `failed` iff some ran agent failed, so the one-line rule satisfies §S8 exactly while treating "another run owns the slot" as success. |
| S6a | **Scraper optional for training + training-scoped env validation (`DataAgent.scraper_required`, `composition.build_training_chain`).** The ATP scraper supplies only the live prediction SLATE; TRAINING consumes Sackmann `final` history only. `DataAgent` takes `scraper_required: bool = True`: when `False`, the scraper's OPERATIONAL incompleteness (failures>0/`complete=False`, e.g. the Cloudflare 403 the adapter absorbs WITHOUT raising) is dropped from the success gate (`ok = not errors and sackmann_ok and odds_ok and owm_ok and (scraper_ok or not scraper_required)`). An UNEXPECTED scraper EXCEPTION (DI/parse/attr bug) STILL surfaces as an `AgentError` in BOTH modes (the §L2 unexpected-bug backstop is never masked — Codex finding) — `scraper_required` relaxes only the operational-completeness gate, never the exception path. `build_training_chain` builds `data_agent(scraper_required=False)`; `build_daily_chain` keeps the default `True`. Relatedly `build_training_chain` validates ONLY the `database`+`sources` env deps (`validate_environment(config, deps=…)`), never the briefing-only SMTP/Anthropic (or RAG Qdrant/Voyage) secrets it does not construct — so `train` runs in prod without them. | atptour.com sits behind Cloudflare and returns HTTP 403 even with a real browser UA (no HTML to parse — §K5/§K6); under §L2 this drops Data to `partial` and skips Research/Modeling, blocking ALL training. The scraper is a prediction-time source, so decoupling it from training lets a model train from Sackmann while the scraper-dependent daily slate stays correctly gated. The daily run still REQUIRES the scraper (an empty slate → no predictions/briefing, §N4) until it is solved (headless browser via the Playwright dependency, or an official slate feed — future extension). Regression tests: `TestScraperOptional` (DataAgent), the `scraper_required` assertions in `TestBuildTrainingChain`/`TestBuildDailyChain`, and `TestTrainingScopedValidation`. |
| T1 | **Matchstat is the canonical daily slate source, replacing the Cloudflare-blocked ATP scraper.** New `adapters/matchstat/` package implements the new `core.contracts.ScraperAdapter` Protocol (`fetch() -> ScraperFetchResult`); `composition.build_daily_chain` wires `MatchstatScraperAdapter` in the scraper-factory slot. `adapters/atp_scraper/` is RETAINED but the live scraper client is no longer instantiated by composition (the parser/§K6 zero-parse guard test corpus stays green as a regression baseline). `build_training_chain` filters `MATCHSTAT_API_KEY` out of its validation deps so training (Sackmann history only) works without a matchstat subscription — extends §S6a. | The scraper was permanently 403'd; the daily run produced zero predictions before this swap. Matchstat is a clean JSON API with the cheapest unblock path and preserves the abstract `ScraperAdapter` seam — DataAgent's `_step_scraper` is unchanged. Regression tests: `TestT1MatchstatWiring` (composition slot + prod-key requirement); `TestHappyPath` (adapter writes `source='matchstat'`). |
| T2 | **Fixtures fetched as `GET /tennis/v2/atp/fixtures/{YYYY-MM-DD}?include=round,tournament.court,tournament.rank,h2h&filter=PlayerGroup:singles;TourRank:1,2,3,4&pageSize=50`.** `status='scheduled'` for unplayed fixtures, `'live'` iff `live` field is truthy, **never `'final'`** from a fixture (a `result` string on a fixture is informational only — `TestT3FixturesNeverInferWinner.test_fixture_never_status_final`). | Matches the §C3 included-tiers policy and the §C4 status filter at ingest. Tier IDs derived from §T5. |
| T3 | **`player1` convention flip is THE most dangerous bug.** `parser.parse_fixtures_page` (fixtures, forward-looking) **NEVER** infers a winner from `player1` — `winner_player_id IS NULL` on every fixture row. `parser.parse_historical_match` (past-matches / tournament/results / h2h/matches) **ALWAYS** treats `player1` as the winner — `winner_player_id == player1.id`, `status='final'`. The two parser functions are deliberately distinct so a future refactor cannot silently mix the conventions. | Silently treating a fixture's `player1` as the winner would corrupt every downstream prediction. Regression tests: `TestT3FixturesNeverInferWinner` (3 cases) + `TestT3HistoricalPlayer1IsWinner`. |
| T4 | **`tennis.utils.score_parser` is the SINGLE canonical score-string parser** for both Sackmann and matchstat. `parse_score(raw) -> ParsedScore` (sets, tiebreaks, deciding-set, retirement, walkover). Only COMPLETE sets count toward `sets_p1`/`sets_p2`: 6-{0..4}, 7-5, or 7-6 (tiebreak). Incomplete sets at retirement (e.g. `"4-2 RET"`) belong to neither player's set count. The parser knows NOTHING about who won the match — caller maps "first-listed in score" ↔ winner/p1/p2. Malformed input raises `ScoreParseError`; never silently zero-fills. | Both sources serialize scores identically; a duplicated parser drifts. Built this session even though score reading is not yet wired (the deferred `pressure`/`tournament_form` families consume it). 25 regression tests covering straight sets, tiebreaks, deciding sets, retirements, walkovers, malformed input, the first-listed↔winner contract. |
| T5 | **`TourRank` IDs hardcoded as `1=GS, 2=Masters1000, 3=ATP500, 4=ATP250`** in `config.sources.matchstat.tier_ids`, used verbatim as the fixtures `filter=TourRank:…` filter expression and as the parser's `_TIER_NAME_TO_ENUM` lookup keys (`Grand Slam`/`Masters 1000`/`ATP 500`/`ATP 250`). A startup §T5 assertion against `/tennis/v2/ranking` (Miscellaneous lookup) is **deferred** — `tier_ids` in YAML pinned today; CLAUDE.md "Carry-forward from T". | These IDs are matchstat's pre-existing classification of the §C3 included tiers — pinning them in YAML matches today's behavior. A future startup assertion will fail loud on rename. |
| T6 | **Free-tier quota: 500/month, 100 req/min/IP.** `MatchstatClient` maintains an in-process monthly counter persisted via `IngestWatermarkRepository.upsert` under `(source='matchstat', scope='quota')` with cursor `{month_key: "YYYY-MM", count: N}`. Stale-month check (cursor month < UTC current month via injected `Clock`) resets to zero on first read. Warns at `quota_warn_at` (400), raises `MatchstatQuotaExhaustedError` at `monthly_quota` (500) BEFORE the 501st call goes out. The slate adapter catches this at its boundary → `failures > 0` + `complete = False` + dead-letter; never propagates so the §L2 gate drops the run to `partial` instead of crashing. | 500/month is binding; exceeding it triggers 429 + subscription dunning. The hard-raise is the only correct failure semantics; the 400-warn is the operator's pre-trigger (the §Q Monitor envelope picks up `matchstat_quota_*` warnings). Regression test: `TestT6QuotaExhausted`. |
| T7 | **API string fields requiring careful parsing:** `odd1`/`odd2` are STRINGS not floats → `Decimal \| None` via pydantic field validator (empty / null → `None`). `seed1`/`seed2` are UNIONS of numeric (`"3"`) AND seed codes `"WC"\|"Q"\|"LL"\|"PR"\|"ALT"`, **stored verbatim** plus a parsed `seed_numeric: int \| None` derived via model_validator (codes → `None`, **NEVER `0`** — that would compare equal to a real 1-seed). `best_of` is `"3"`/`"5"` string → `int \| None`. `birthday` is `"YYYY-MM-DD"` → `date \| None`. | Type-laundering at the model boundary is the single best place to absorb upstream string-typing; failing fast in pydantic beats silent garbage in features. Regression tests: `TestT7StringTyping` (5 cases) + `TestModelTyping`. |
| T8 | **Pagination contract: ALWAYS paginate every page via `hasNextPage`.** `MatchstatClient.paginate_all(fetch)` is the centralized helper — yields items from every page until the upstream returns `hasNextPage=False`, capped at `_MAX_PAGES=200` to defend against an upstream loop bug. The slate adapter inlines this loop (so per-page errors map to per-page dead-letter, not a single bulk failure). `pageSize=50` chosen for fixtures (a Grand Slam day has 20+ singles matches per round). | A page-1-only fetch silently truncates the slate; truncation is undetectable from the result envelope (no `total` field) until a downstream count check catches it. |
| T9 | **Authentication via two RapidAPI headers injected by `MatchstatClient` on every request:** `X-RapidAPI-Key` from `MATCHSTAT_API_KEY` env (resolved at composition via `read_required_env` for the daily chain; `None` for training) + `X-RapidAPI-Host` from `config.sources.matchstat.host`. Missing key in daily → `MissingEnvironmentError` from `validate_environment`. 401/403 → `MatchstatAuthError(AdapterError)` — retrying cannot help; the runbook diagnosis is "check `MATCHSTAT_API_KEY` and `host`". | RapidAPI requires both headers; missing the host returns 403 with a confusing "subscribe to API" message. The typed auth error makes the runbook diagnosis instant. The key is never logged (never in a structlog kwarg, never in a raise message, never in a dead-letter payload). |
| T10 | **`matchstat_id INTEGER NULL` column on `matches`** (migration 014, additive, no backfill) records the matchstat fixture id for forward joins (e.g. tournament-specific match stats). Indexed partial-where-not-null. Set on every matchstat-written `MatchRow`; the `MatchRepositoryImpl.upsert` `ON CONFLICT DO UPDATE` set_ uses `COALESCE(EXCLUDED.matchstat_id, matches.matchstat_id)` so a later non-matchstat write with NULL never clobbers a previously-set value. **Does not participate** in the §K1 `match_id` hash — matchstat is a source attribute, not a match-identity attribute. | A second source coming online must NEVER fork the canonical `match_id`. The new column is a sidecar pointer, not a key participant. Regression tests: `test_match_row_matchstat_id_populated` (rows) + `TestHappyPath.test_writes_one_matchrow_with_matchstat_metadata` (adapter). |
| T11 | **§C2 DOB+country tiebreaker enrichment via `MatchstatSearchEnricher`** — wraps `client.search(name)`, returns first `player`-type hit with `birthday`; per-`(name)` in-process cache; quota-exhausted → `None` so the resolver always falls through to Sackmann-only (the enricher NEVER propagates `MatchstatQuotaExhaustedError`). `SackmannResolver` integration is **deferred** to a follow-up session — the sidecar is wired into composition but the resolver's ctor dep is not yet threaded. CLAUDE.md "Carry-forward from T". | DOB+country was previously hard to populate (Sackmann CSVs lack DOB for the long tail); the search endpoint closes the gap without changing §C2 priority. §H6 invariant preserved: `player_aliases` remains the sole alias store. |
| T12 | **`tournament/calendar/{year}` populates `venues.lat/lon` directly via `MatchstatVenueGeocoder.populate(year=Y)`** — `_consume_tournament_calendar` paginates the year, upserts `VenueRow` per `(site, countryAcr)` with `latitude`/`longitude` (no `altitude_m` from upstream). Replaces `config/venue_coords.yaml` long-term. v1 keeps the YAML on disk as the §L11 safe fallback (warning, not failure); deletion is gated on the calendar step being verified populating coords on at least one real production run. The DataAgent `_step_matchstat_calendar` wiring is **deferred** (Phase D carry-forward) — the sidecar is constructed via factory but not yet invoked from DataAgent.run(). | A live geocoding feed retires the manual file without losing the safe fallback. The YAML's 58 entries stay authoritative until the calendar pass is observed populating coords end-to-end in prod. |
| T13 | **`ranking/singles?filter=RankingDate:{YYYY-MM-DD}` seeds the v1 `player_rankings` table via `MatchstatRankingsSeeder.populate(ranking_date=monday)`** — paginates Top-N (`pageSize=200`), upserts per `(player_id, ranking_date)`. Player resolution joins via `player_aliases.(source='matchstat'\|'sackmann')`; unresolvable rows are SKIPPED (no aggressive shadow minting from the ranking endpoint — too speculative for v1). The DataAgent `_step_matchstat_rankings` wiring is **deferred** (Phase D carry-forward). The §M11 extractor itself does NOT change this session — date-EXACT (not `latest_before`) is the future pressure-features session. | The rankings table was sparsely seeded (Sackmann weekly only); the new step gives `latest_before` something to find without coupling extractor changes to adapter wiring. The pressure-features session that consumes the same endpoint for points-defending will tighten §M11 to date-EXACT. |
| T14 | **New h2h family columns** (catalog + extractor): `p{1,2}_h2h_deciding_set_wins / h2h_tiebreak_wins / h2h_comeback_wins`, mapped 1:1 from matchstat `h2h/stats.player{1,2}Stats.{decidingSetWin, tiebreakWon, firstSetLoseMatchWin}`. The `MatchstatH2hStatsClient` sidecar (per-pair cache, C14 NULL on miss / `matchesCount=0` / quota) is built and wired into composition. The `H2HExtractor` extension itself (6 new keys + `feature_specs` rows + round-trip test update) is **deferred** to a follow-up session — Phase E carry-forward. | Matchstat precomputes clutch H2H signals we'd otherwise need to score-parse for every prior match; wire them straight through once the extractor change lands. Field-name mapping will be pinned by a regression test in the follow-up so an API rename fails loud. |

**§Q (Monitor Agent observability locks).** Next free namespace after §O — §P and
§R are skipped per the §M convention (they collide with build-phase `P#` and
session/validator `R#` labels). Q1–Q8 lock the post-pipeline Monitor and, like §N,
share the `## M.` decision table.

**§S (Orchestrator-wiring locks).** The deferred DI + cron-shim session: the
multi-agent sequential loop (§S4), the `train`/`run` CLI entrypoints (§S2/§S3) +
composition root (§S7), the §N5 resolution (§S5 — `briefing_deliveries`,
migration 013), the bootstrap contract (§S6), the §S8 exit codes, and (first
live-run session) §S6a — the training chain decouples from the Cloudflare-blocked
ATP scraper and validates only DB + source-adapter secrets. Next free single
letter (§P/§R skipped, §O is OWM); like §N/§Q the S-rows share the `## M.`
decision table.

**§T (Matchstat slate adapter locks).** The unblock-the-daily-run session: the
ATP scraper is permanently Cloudflare-blocked (HTTP 403 even with a real browser
UA, verified live), so the daily slate is empty and Modeling has nothing to score.
§T1–§T14 swap `MatchstatScraperAdapter` (RapidAPI `tennis-api-atp-wta-itf`) into
the scraper-factory slot behind a new abstract `core.contracts.ScraperAdapter`
Protocol — nothing downstream changes. §T1 daily-slate; §T2 fixtures filter +
status; §T3 the player1 convention flip (the single dangerous bug — fixtures
NEVER infer a winner, historical ALWAYS treats player1 as winner); §T4 canonical
`tennis.utils.score_parser`; §T5 TourRank IDs; §T6 500/month quota guardrails;
§T7 string-typed field laundering; §T8 mandatory pagination; §T9 two-header auth;
§T10 sidecar `matchstat_id` column (migration 014); §T11/§T12/§T13/§T14 the four
sidecars (search enricher, calendar geocoder, rankings seeder, h2h-stats client)
— their DataAgent/Resolver/H2H-extractor wiring is intentionally deferred and
carried forward in CLAUDE.md. Sharing the `## M.` decision table per the
§N/§Q/§S convention.
...
| 4 (M1a) | [hash TBD] | Modeling Agent **M1a** — data assembly + walk-forward CV + base-learner training + the `ModelingAgent` skeleton (968→1052 unit tests, +84; +1 ML stack installed). New `src/tennis/models/`: `feature_set.py` (resolve the model X — the whole `market` family + `forecast_uncertainty_bucket` + `_BACKTEST_ONLY_KEYS` EXCLUDED, §M20b/§M21a/b — + `feature_hash`=SHA-256(sorted keys), §M20d), `assembly.py` (`feature_matrix`+labels→(X,y,meta); drop walkover **and** invalid-`winner_id`, keep retirement, §M21f; native categorical + native missing, §M20a), `splits.py` (walk-forward CV with the calibration tail carved BEFORE folds + both-sides tournament embargo incl. the train↔tail seam, §M20c/§M21c/e), `base_learners.py` (XGB/LGBM native-categorical + train-subsplit early stop §M21d; OOF + logloss/brier; single-class folds skip+count, per-fold heartbeat §L7), `noise.py` (H1 `apply_noise()` no-op stub), `artifacts.py` (joblib persistence + µs-resolution version mint, §M20d). New `agents/modeling/agent.py` (`ModelingAgent`): precondition (`research` succeeded) → assemble → split → CV → final train → joblib serialize → `model_registry` insert + **activate** (≤1-active flip; base-only `algo`, superseded by M1b); zero-OOF/NaN-metric **activation gate**; succeeded/failed (insufficient-data / DB) status, `redact_text` causes (§L10). Config: `ModelingSection.artifact_dir` (`min_length=1`), `CalibrationConfig.tail_days` + `SplitsConfig.min_train_seasons` `gt=0`. `pipeline.py`: `insufficient_training_data`/`modeling_db_error` → `_FATAL_CODES`. `specs.py`: `family_feature_keys()` helper. New locked decisions **M20** + **M21** (§M; locked pre-build, with the feature_hash dtype-drift deferral note appended post-review). Auto-review ✅ 0 CRITICAL (3 polish fixes: `gt=0`/`min_length=1` guards, dead test param). Codex adversarial-review (1 CRITICAL / 2 HIGH / 3 MEDIUM → 5 fixed + 1 deferred): per-fold heartbeat in `cross_val_oof` (§L7); single-class folds skip-in-CV / raise-on-final / `predict_p1` handles `(n,1)`; zero-OOF activation gate (fail clean, zero writes); µs version (no same-second PK collision); invalid-winner drop+count; **feature_hash dtype-drift DEFERRED** to M1b/Monitor (§M21 note — keys-only per §M20d). Env: the venv lacked the declared ML stack (numpy/pandas/scipy/scikit-learn/joblib/xgboost/lightgbm) → installed; `pyproject.toml` per-file ruff ignores for the ML `X` naming convention. |

| 4 (M1b) | [hash TBD] | Modeling Agent **M1b** — scoring half: stacking + calibration + edge + Kelly + `predictions` write + real H1 noise + a prediction MODE (1052→1112 unit tests, +60). New `src/tennis/models/`: `stacker.py` (logistic meta-learner over the per-learner OOF matrix; single-class-OOF guard), `calibration.py` (Platt/isotonic on the held-out tail; empty/small/single-class → degraded passthrough → `partial`), `edge.py` (Shin + proportional edge vs payload implied; C9 NULL), `kelly.py` (fractional Kelly; per-match cap + same-day pro-rata cap §M24/H12; Shin→proportional fallback; NULL no-odds / 0.0 below-threshold), `metrics.py` (ECE + Kelly-ROI backtest). Extended: `noise.py` (real seeded per-bucket §M22 injection, training-only), `base_learners.py` (`CVResult` per-learner `oof_xgb`/`oof_lgbm` + `oof_matrix()`), `assembly.py` (prediction assembler bound to `model.feature_set` with §M23 pinned categories + `extract_categorical_categories`), `artifacts.py` (`TrainedModel` carries stacker+calibrator+`categorical_categories`; algo `xgb+lgbm_stack_platt`). New `core/weather_uncertainty.py` (§M15 band logic moved out of `conditions.py` so `noise.py` shares it without a models→agents import; `conditions.py` re-exports). `agents/modeling/agent.py`: `mode` ctor flag (training/prediction); prediction loads active model → score → edge → Kelly → upsert with per-match fault isolation, `p1_implied_close`/`odds_drift_to_close` NULL globally (§M19); `pipeline._FATAL_CODES` += `no_active_model`, `feature_set_mismatch` (calibration_degraded deliberately OUT → `partial`). Config: `ModelingSection.random_seed=42`. New locked decisions **M22** (noise sigma + CV-on-noised-frame), **M23** (categorical code pinning), **M24** (Kelly same-day cap), **M25** (prediction-mode hardening). Auto-review (RUN REVIEW): ✅ 0 CRITICAL. Codex adversarial-review (0 CRITICAL / 3 HIGH): **HIGH** feature-set config-vs-artifact drift → fail-fast `feature_set_mismatch` + artifact-bound matrix read (FIXED, §M25a); **HIGH** batch predict outside per-match isolation → per-row scoring inside the try/except + non-finite-prob guard (FIXED, §M25b/c); **HIGH** noise-before-CV → triaged PARTIALLY-VALID/design-judgment, kept (consistent noising matches the deployed base learners; documented in §M22) — 2 fixed + 1 documented. |

| 4 (B1) | [hash TBD] | Briefing Agent **B1** — the fourth/final pipeline agent: the day's `predictions` → Claude narrative → edge/Kelly email (1112→1153 unit tests, +41). New `agents/briefing/`: `agent.py` (`BriefingAgent(Agent)`, precondition `modeling` succeeded; §N3 midnight-UTC slate window; active-`model_version` filter; per-side Shin→proportional edge classification; one degrade-safe LLM call; per-match context fault isolation §L2; SMTP send; §N4 status), `render.py` (pure: C13 disclaimer on every Kelly line, C9 NULL→"no market", §M24 NULL-vs-0.0, closing fields structurally absent §M19/§15.4, `subject_template` fill), `llm_client.py` (`LlmClient` Protocol §N2 + `AnthropicLlmClient`), `email_client.py` (`EmailClient` Protocol §N2 + `SmtpEmailClient`; env creds/recipients via `read_required_env`, STARTTLS, `redact_text` §L10). `core/errors.py`: `BriefingError`/`BriefingLlmError`/`BriefingEmailError`. `pipeline._FATAL_CODES` += `no_qualifying_predictions`, `smtp_send_failed`, `briefing_db_error` (`briefing_partial` deliberately OUT → `partial`). New locked decisions **N1–N5** (§N): N1 RAG deferred, N2 injectable LLM/Email Protocols, N3 slate window, N4 status semantics (NULL-edge-only slate → `failed`/zero-email), **N5 email-delivery idempotency DEFERRED v1**. Anthropic SDK 0.104.1 already in the venv (no install). Auto-review (RUN REVIEW): the first fire reviewed the stale M1b diff (new files were untracked); resolved with `git add -N`. Codex adversarial-review (0 CRITICAL / 2 HIGH, both triaged): **HIGH F1** — `_classify` row-level method selection → **per-side** Shin→proportional fallback so a real value side is never dropped (FIXED + 5 mixed-permutation regressions; confirmed defensive — `compute_edges` writes Shin/proportional atomically per row, edge.py:59-64); **HIGH F2** — non-idempotent SMTP send → **DEFERRED as §N5** (the §L7 advisory lock already blocks concurrent double-send; a durable outbox needs a migration + its own decision, owner = the orchestrator-wiring session). |

| 4 (§S) | [hash TBD] | **Orchestrator wiring** — the multi-agent loop + CLI + composition root + §N5 resolution (1177→1209 unit tests, +32; +2 Docker-gated integration). **§N5 RESOLVED (§S5):** migration `013_briefing_deliveries` + `BriefingDelivery` ORM/Row/Protocol/`BriefingDeliveryRepositoryImpl` (INSERT … ON CONFLICT DO NOTHING, `UNIQUE(briefing_day_utc, model_version)`); `BriefingAgent` gains a `briefing_delivery_repo` ctor dep, a get-before-send idempotency skip (`already_delivered` → succeeded/zero-email, before the LLM), and a best-effort record-after-send. **§S4 multi-agent loop:** `DailyPipeline(agents: Sequence[Agent])` runs the chain under ONE `run_id`/lock/orphan-sweep; precondition-not-met → skip (no row, warn, continue); `run()`-raise → record `failed` + continue (Monitor still runs); `run_once()` returns the aggregate status. **§S7 composition root:** new `composition.py` (`build_training_chain`/`build_daily_chain`) + new `HttpxClient` transport (`adapters/http_client.py`, the one missing concrete `core.contracts.HttpClient`) — wires `PostgresSessionFactory` → repos → adapter-factory closures → LLM/SMTP clients from `AppConfig`+env (`validate_environment` fail-fast; lazy network → unit-testable). **§S2/§S3/§S8 CLI:** new `cli.py` (typer `train`/`run`) + `__main__.py` (`python -m tennis`); exit 1 iff the aggregate is `failed`. New locked decisions **S1–S8** (§S). `typer` installed into the venv (declared in `pyproject` but absent). Tests: `test_composition.py` (agent-set+modes, missing-secret raises), `test_cli.py` (command→builder + §S8 exit matrix), `adapters/test_http_client.py` (get/post via `httpx.MockTransport`), `briefing/test_agent.py` (+4 §S5 idempotency), `storage/test_impl.py` (+`BriefingDeliveryRepository`), `orchestrator/test_pipeline.py` (chain + data-fail-skip-Monitor-runs + run()-raise-continue + **§S6 bootstrap** regression), integration `test_repositories.py` (+round-trip & ON-CONFLICT no-op). Auto-review (RUN REVIEW): 0 CRITICAL / 2 HIGH → H1 per-agent `started_at` uses the real clock not pinned `as_of` (§L4); H2 `_singleton_lock` → canonical `with` (both fixed). Codex adversarial review (0 CRITICAL / 1 HIGH / 1 MEDIUM) + 1 reviewer-found gap, triaged: **HIGH** swallowed record-after-send duplicate-send = the locked §S5 residual (KEPT documented; hardened: log at ERROR as pageable `briefing_delivery_record_failed`); **MEDIUM** the H2 plain-`with` dropped the typed lock-acquisition error → RESTORED `PipelineStartupError` normalization (§L2) + a regression test; **GAP Codex missed** — `run_monitor_post_briefing` was defined but consumed NOWHERE → `build_daily_chain` now honors it (omits Monitor when `False`) + a composition test (§Q5/§Q7/A13). 1207→1209 unit tests. |
| 4 (Monitor) | [hash TBD] | Monitor Agent **§Q** — the post-pipeline 5th stage (AGENTS.md A13), observability that is **NOT** part of the 4-agent prediction chain (1153→1177 unit tests, +24; +2 Docker-gated integration). New `agents/monitor/{__init__,agent}.py` (`MonitorAgent(Agent)`, `name="monitor"`, **`preconditions=()`** — runs regardless of upstream status; the `run_monitor_post_briefing` gate lives in the orchestrator): per-window ECE/PSI/ROI over `config.monitor.windows_days`, realized-outcome join (`status='final'` + `winner_id==p1_id`; not-final / corrupt-winner → pending, excluded-but-counted), rolling-previous-window PSI (anchor = **NEXT midnight UTC** so the run day's predictions are included), §Q6 metrics envelope returned in `AgentResult.metrics` (orchestrator persists via `update_status`; no direct run-row write). New `models.metrics.population_stability_index` (equal-width [0,1] bins, epsilon-floored). New `MatchRepository.list_for_ids` (Protocol + `MatchRepositoryImpl` + empty `{}` fast-path, single IN-list, typed `StorageError`) — the §M18 bulk dual retiring the per-prediction `get` N+1. `pipeline._FATAL_CODES` += `monitor_db_error` (only fatal Monitor code; `monitor_partial`/`monitor_no_active_model` → `partial`). New locked decisions **Q1–Q8** (§Q). Auto-review (RUN REVIEW): ✅ 0 CRITICAL / 1 HIGH — **H1** window off-by-one (start-of-day midnight anchor excluded the run-day slate) → FIXED: next-midnight anchor + regression test. Codex adversarial-review (0 CRITICAL / 2 HIGH / 1 MEDIUM, all triaged VALID + fixed): **HIGH F1** — `monitor_db_error` returned a non-§Q6 payload → return the §Q6 envelope on EVERY terminal state + test; **HIGH F2** — per-prediction `get` N+1 → bulk `list_for_ids` (one read/window) + perf-guard unit test + integration test; **MEDIUM F3** — UTC not enforced → normalize `as_of.astimezone(UTC)` before window math + non-UTC test. Lint: removed an unused `Sequence` import + lowercased a test name (the `TC001`/`RUF002` baseline matches the shipped briefing agent and is left consistent). |

| 4 (§T matchstat) | [hash TBD] | **Matchstat slate adapter — unblock the daily run** (1221→1293 unit tests, +72; +53 build + +19 Codex-triage). The Cloudflare-blocked ATP scraper produced zero predictions on the first live run; this session swaps the daily slate source to matchstat (RapidAPI `tennis-api-atp-wta-itf`) at the same factory seam so nothing downstream changes. **Foundations:** new `core.contracts.ScraperAdapter` + `ScraperFetchResult` Protocols (§T1); `MatchstatAuthError` + `MatchstatQuotaExhaustedError` errors; migration **014** (`matches.matchstat_id INTEGER NULL`, partial index, §T10); new `MatchstatSource` pydantic submodel + `sources.matchstat:` config block; `agents/data/agent.py:63` `ScraperFactory` retyped to the abstract Protocol so the DataAgent code is unchanged. **Score parser:** new `tennis.utils.score_parser` (`ParsedScore`+`parse_score`, §T4) — canonical for both Sackmann and matchstat, complete-sets-only counting, retirement/walkover/tiebreak detection; 25 regression tests. **Adapter package:** new `adapters/matchstat/{__init__,models,client,parser,adapter,sidecars}.py`: pydantic v2 DTOs with §T7 string-typed field validators (`odd1/odd2` → `Decimal`, `seed1/seed2` verbatim + `seed_numeric`, `best_of` int, `birthday` date); `MatchstatClient` with §T9 two-header auth + §T6 quota counter persisted via `ingest_watermarks` (warn@400 / raise@500 / stale-month reset) + §T8 `paginate_all` helper + 401/403→`MatchstatAuthError` / 429→`RateLimitError` / 5xx→`UpstreamUnavailableError` mapping + lookup cache; `parser.py` enforces the §T3 player1 convention split (`parse_fixtures_page` never infers a winner; `parse_historical_match` always treats player1 as winner); `adapter.py` runs the slate over `[today, today+lookforward_days]` and writes via `MatchRepository.upsert` with `source='matchstat'`, `source_uid='matchstat:{fixture_id}'` (§K2), `matchstat_id` populated (§T10), `match_date` snapped to the §K3 tournament-week Monday. **Composition:** `_Wiring` builds one shared `MatchstatClient` (one quota counter); `_scraper_factory` swap is the §T1 unblock; new `_matchstat_calendar_factory`/`_matchstat_rankings_factory` for §T12/§T13 (built but **not yet consumed** by DataAgent — Phase D carry-forward); `build_training_chain` filters `MATCHSTAT_API_KEY` out of validation deps (extends §S6a); `build_daily_chain` requires it. **Storage:** `MatchRow.matchstat_id` + `Match` ORM column; `MatchRepositoryImpl.upsert` `COALESCE(EXCLUDED.matchstat_id, matches.matchstat_id)` so a non-matchstat writer never clobbers it. **Tests:** 25 score_parser + 22 matchstat parser (incl. §T3 fixture+historical pinning + §T7 string-typing + §C3 tier-drop) + 3 matchstat adapter (happy path + live status + §T6 quota degrade) + 2 composition (`TestT1MatchstatWiring`: matchstat-adapter wired + prod requires key) + 1 storage row matchstat_id round-trip. New locked decisions **§T1–§T14** (in the §M decision table per the §N/§Q/§S convention). **Deferred to a follow-up §T-2 session (CLAUDE.md "Carry-forward from T"):** §T11 SackmannResolver search-enricher ctor wiring; §T12 DataAgent calendar step invocation; §T13 DataAgent rankings step invocation; §T14 `H2HExtractor` ctor dep + 6 new keys + `specs.py` rows + round-trip test; §T5 startup TourRank assertion; live-API fixture-capture script (`scripts/capture_matchstat_fixtures.py`). **Auto-review (RUN REVIEW):** ✅ 0 CRITICAL — 4 findings (H1 PIT-test assertion + H2 lookup-cache miss-on-restart + M1 calendar tier filter + M2 leaked Anthropic key in `.env.example`) all FIXED before Codex. **Codex adversarial review:** 0 CRITICAL / 2 HIGH / 3 MEDIUM, ALL VALID + FIXED. **HIGH 1** — `matchstat_id` was silently dropped on the `update_live_fields` reconciliation path (a Sackmann-first row that matchstat later sees live would keep `matchstat_id=NULL` forever): added optional `matchstat_id: int | None = None` to the `MatchRepository.update_live_fields` Protocol + impl (NULL-safe; mirrors the upsert COALESCE), and the adapter now passes `fx.matchstat_id` on the merge path. **HIGH 2** — relaxed §K6 zero-parse guard could silently mis-report `complete` on an empty slate if upstream filter/parsing drifted: added `pages_with_data` + `successful_calls` to `_Batch`, and `fetch()` now fails closed when the whole-run total is zero while the API was responding 200 (per-date zero stays a legitimate INFO). **MEDIUM 3** — duplicate of HIGH 1 from the impl side (same fix). **MEDIUM 4** — `composition._Wiring.__init__` had a bare `except Exception` around `read_required_env` that would swallow programmer bugs and silently set `api_key=None`: narrowed to `except MissingEnvironmentError` so wiring defects fail fast at construction (matchstat key remains optional in training per §T1 extension of §S6a). **MEDIUM 5** — `matchstat/client.py` had zero unit tests despite hosting the load-bearing quota + auth + cache code: new `tests/unit/adapters/matchstat/test_client.py` (15 tests via `httpx.MockTransport`) pinning §T9 two-header injection, 401/403 → `MatchstatAuthError`, 429 → `RateLimitError`, 5xx → `UpstreamUnavailableError`, the §T6 quota counter (increment, persistence, restart-load, warn@N-1, raise BEFORE the over-cap call, stale-month reset on UTC month change, `quota_remaining` property), and the §T6 lookup cache fresh-watermark/stale-watermark-refresh/bad-cursor-fallthrough/watermark-read-failure/naive-`fetched_at` rejection paths. |

| 4 (first-live) | [hash TBD] | **First live end-to-end run + deployment hardening** (1209→1221 unit tests, +12). First execution against real Postgres (Supabase) + real external services. **DB:** alembic 001→013 applied (20 objects incl. `briefing_deliveries`); `ops/alembic.ini` `script_location`/`prepend_sys_path` re-anchored with `%(here)s` so the documented "run from repo root" actually resolves. **Mirror:** new `scripts/sync_sackmann_mirror.py` — git clone/pull + `os.utime` dir-mtime refresh that defeats the §C5 staleness halt on a no-op pull (900s timeout; missing-git/timeout→`GitError`). **ops/ deploy surface:** `lib.ps1` (robust `.env` loader `Import-DotEnv` — preserves the space-bearing SMTP password, strips inline comments — + `Invoke-Tennis`), `run_train.ps1`/`run_daily.ps1` wrappers, `tennis-run.xml` (Task Scheduler 06:30 **UTC**, NOT auto-registered, §S7), `README.md` runbook. **Key finding:** atptour.com is Cloudflare-blocked — HTTP 403 even with a real browser UA (verified live) — so the scraper supplies no slate and, under §L2, dropped Data to `partial` and blocked ALL training. **Fix — new locked decision §S6a:** `DataAgent.scraper_required` flag; `build_training_chain` sets it `False` so the scraper's OPERATIONAL incompleteness no longer gates training (Sackmann-`final`-history only), while an UNEXPECTED scraper exception STILL surfaces as an `AgentError` in both modes; `build_training_chain` also validates ONLY DB+source-adapter secrets (it never constructs Briefing). Live services verified: DB✓ Odds✓(200/500-quota) OWM✓(200) scraper✗(Cloudflare). The first full backfill is a multi-hour **resumable** job over remote Supabase (measured ~80ms/write) — documented in the runbook, not run to completion in-session; Docker absent so integration-chain tests skipped. Auto-review (RUN REVIEW): H1 training-scoped validation + §S6a doc FIXED; H2 ("no branch field") / M2 ("Playwright not in pyproject") triaged FALSE POSITIVES (`config.py:135`, `pyproject.toml:33`) — branch-field contract locked by a test instead of a needless hardcode. Codex adversarial-review (0 CRITICAL / 2 HIGH / 1 MEDIUM, all triaged VALID): **HIGH** machine-specific `local_mirror_dir` → kept OUT of the commit (local-only working-tree change; committed default stays repo-relative `.cache/sackmann/mirror`); **HIGH** optional-scraper `except` masked real bugs → narrowed so unexpected exceptions always surface (§S6a); **MEDIUM** sync script no timeout / uncaught missing-git → 900s timeout + `FileNotFoundError`/`TimeoutExpired`→`GitError` (+2 regression tests). |

Next session resumes at **§T-2 sidecar consumer wiring + first real daily run**. §T1 swapped the slate to matchstat at the factory seam and 1293 unit tests are green, but four sidecars (built and composition-wired) still have no consumer in the daily run. §T-2 wires them: (1) `SackmannResolver.search_enricher` ctor hook for §C2 DOB+country tiebreaker enrichment; (2) `DataAgent._step_matchstat_calendar` (calendar → `venues.lat/lon`, retiring `venue_coords.yaml`); (3) `DataAgent._step_matchstat_rankings` (date-filtered Top-200 → `player_rankings`, seeding §M11 `latest_before`); (4) `H2HExtractor.h2h_stats_client` ctor dep + 6 new keys + `specs.py` rows + round-trip test (§T14). Also: the §T5 startup TourRank assertion against `/tennis/v2/ranking`; the live-API fixture-capture script (`scripts/capture_matchstat_fixtures.py`) to replace synthesized JSON; and the §T6 quota-burn metric exposed in the §Q Monitor envelope. THEN the previously-deferred deployment work: (a) run the first **training backfill** to completion via `ops/run_train.ps1` — a multi-hour resumable job over remote Supabase (§S6 bootstrap; §S6a means the Cloudflare scraper no longer blocks it); (b) register the **Task Scheduler** job (`ops/tennis-run.xml`, 06:30 UTC — documented, not auto-registered); (c) **subscribe to matchstat (RapidAPI tennis-api-atp-wta-itf) + add `MATCHSTAT_API_KEY` to `.env`** before the first daily run.

**Pre-commit operator action:** rotate the `ANTHROPIC_API_KEY` previously committed in `.env.example` at console.anthropic.com — the placeholder fix in the working tree does NOT remove it from git history. The §S orchestrator-wiring session shipped the sequential multi-agent loop under one shared `run_id` (DataAgent → ResearchAgent(prediction) → ModelingAgent(prediction) → BriefingAgent → MonitorAgent, the §L12 gate now firing end-to-end), the `train`/`run` CLI + composition root (§S2/§S3/§S7), and the §N5 delivery guard (`briefing_deliveries`, §S5) — 1209 unit tests green. The daily pipeline runs ModelingAgent in **prediction** mode; **training** is the separate `python -m tennis train` job that must run first to populate an active model (the §S6 bootstrap: `no_active_model` fails fast → Briefing skipped → Monitor partial). Remaining work is out of repo scope: the **OS scheduler** (cron/systemd invoking `python -m tennis run` at 06:30 UTC + a less-frequent `train`, §S7) and **§N1 Qdrant RAG** (`briefing.rag` present but not consumed; `storage/qdrant/` unbuilt, §E1). Optional future hardening, each its own documented session: the §S5 full outbox-before-send; §M3 `wind_serve_risk`/`altitude_serve_boost` interactions + the forecast-vintage `created_at < as_of` weather filter (§M15); the date-granular rank PIT (§M11); `odds_drift_to_close` (the per-tournament `|line_movement|` aggregate, §M19 — deferred-NULL in v1); the run-level `bulk_read_failures` metric (§M18 / §Q8 — Monitor-owned); `feature_hash` dtype-drift / feature-input PSI (§M21 / §Q8 — Monitor-owned); the warn/alert threshold CLASSIFICATION of ECE/PSI (`config.monitor.calibration`/`drift` — §Q stores raw metrics, downstream alerting classifies); and the optimistic no-vig `1/implied` ROI basis (§Q2 — diagnostic, real vigged-odds ROI a future refinement).
