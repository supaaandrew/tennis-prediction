...
└── tests/
    ...
    ├── unit/                  # 871 tests, all green locally
...
Next session resumes at **R6 — ResearchAgent orchestrator** (`agents/research/agent.py`): the extractor-registry wiring that assembles the now-complete R3–R5b base families (`elo`/`rankings`/`form`/`h2h`/`surface`/`serve_return`/`conditions`) behind one agent, plus two carry-forward MUSTs — (a) a startup invariant asserting `config.features.windows_days` matches the seeded Form catalog (§M12 / CLAUDE.md R4 carry-forward; raise `FeatureContractError` before extraction), and (b) a bulk/prefetch `match_stats` read path for serve/return + a query-count perf guard to retire the §M14 N+1 (Codex R5a carry-forward). The §M3 `wind_serve_risk`/`altitude_serve_boost` interactions (config curves + cross-family serve profile) and the forecast-vintage `created_at < as_of` repo filter remain deferred (§M15). **R7** = fatigue + market signals (plugs into the R6 registry). Remaining glue (DI adapter-factory wiring + cron shim) is the deferred thin layer noted in §5.
