...
└── tests/
    ...
    ├── unit/                  # 782 tests, all green locally
...
Next session resumes at **R5 — serve/return + surface affinity + conditions (weather) extractors** (`research_specs.md` §R5; `spec.md` regenerated by this session's `session-summary` wrap-up): the `features.serve_return`, `features.surface`, and `features.conditions` families — reading `match_stats` / prior matches via the R2 `MatchHistoryIndex` and weather via the conditions joins, registered in `specs._REGISTRY` in lockstep (the §M7 mechanism, now exercised by `elo`/`rankings`/`form`/`h2h`). **R6** then lands the `ResearchAgent` orchestrator (`agents/research/agent.py`) and MUST add the startup invariant that `config.features.windows_days` matches the seeded Form catalog (§M12 / CLAUDE.md R4 carry-forward — the deferred runtime guard for the pinned-vs-config window divergence). **R7** = fatigue + market signals. Remaining glue (DI adapter-factory wiring + cron shim) is the deferred thin layer noted in §5.
