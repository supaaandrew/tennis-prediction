...
└── tests/
    ...
    ├── unit/                  # 832 tests, all green locally
...
Next session resumes at **R5b — conditions (weather) extractor** (`research_specs.md` §R5; the deferred half of R5, `spec.md` regenerated by this session's `session-summary` wrap-up): the `features.conditions` family — the 6 weather fields via `WeatherObservationRepository.nearest_at_or_before(source="owm", max_age_hours=config.features.weather.max_obs_age_hours=3)` (hindcast at training / forecast at decision, never after `start_ts`), `altitude_m` + `indoor` venue/tournament facts, and `forecast_uncertainty_bucket` (from `forecast_horizon_h` via `config.features.weather.uncertainty_bucket_thresholds`; NULL for hindcast); `venue_id IS NULL` → all-NULL row still written (C9) — plus the §M3 `wind_serve_risk`/`altitude_serve_boost` interactions (config-gated by `features.conditions_interactions.enabled`; both need NEW config curves + a cross-family serve profile, a design decision deferred to R5b), registered in `specs._REGISTRY` in lockstep (the §M7 mechanism, now exercised by `elo`/`rankings`/`form`/`h2h`/`surface`/`serve_return`). **R6** then lands the `ResearchAgent` orchestrator (`agents/research/agent.py`) and MUST (a) add the startup invariant that `config.features.windows_days` matches the seeded Form catalog (§M12 / CLAUDE.md R4 carry-forward), and (b) add a bulk/prefetch stats read path for serve/return + a query-count perf guard (§M14 / Codex R5a carry-forward). **R7** = fatigue + market signals. Remaining glue (DI adapter-factory wiring + cron shim) is the deferred thin layer noted in §5.
