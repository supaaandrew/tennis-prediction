...
└── tests/
    ...
    ├── unit/                  # 719 tests, all green locally
...
Next session resumes at **R4 — form / H2H / ranking extractors** (`research_specs.md` §R4; `spec.md` regenerated by this session's `session-summary` wrap-up): rolling win-rate windows, §M1 H2H confidence/recency weighting, and ranking features — reading each player's prior matches via the R2 `MatchHistoryIndex` and registering their families in `specs._REGISTRY` in lockstep (the §M7 mechanism R3 first exercised with the `"elo"` family). R5–R7 specs remain in `research_specs.md`. Remaining glue (DI adapter-factory wiring + cron shim) is the deferred thin layer noted in §5.
